...
class ToolBarClassificationData {
  ...
  constructor(send_toolBarClassificationName: string, send_toolBarClassificationLogo: string) {
  ...
  insertToolBarEditorDict = (newName: string, send_EditorLogo: string, send_EditorFunction: Function, overwrite: boolean) => {
    if (hasKeyFound(newName, this.toolBarEditorDict) && !overwrite) {
      return;
    }
    const newObj = new ToolBarEditorData(newName, send_EditorLogo, send_EditorFunction);
...
class ToolBarEditorData {
  ...
  constructor(send_toolBarEditorName: string, send_editorLogo: any, send_editorFunction: Function) {
...
const SetupToolbar = () => {
  ...
  // ここでhooksを使える
  const [toolBarClassificationArray, toolBarClassificationArraySetState] = useState<{ [name: string]: ToolBarClassificationData }>({}); //これで仕分け
  ...
  const insertToolBarClassificationArraySetStateValue = (
    ...
    // const newID = getUUID()
    const newObj = new ToolBarClassificationData(send_toolBarClassificationName, send_toolBarClassificationLogo);
  ...
  const insertToolBarEditorDictSetStateValue = (
    ...
    const copyToolBarClassification = Object.assign(toolBarClassificationArray);
    copyToolBarClassification[send_toolBarClassificationName].insertToolBarEditorDict(send_toolBarEditorName, send_EditorLogo, send_EditorFunction, overwrite);
    toolBarClassificationArraySetState(copyToolBarClassification);
  };

  const operationEditorStatus = (send_toolBarClassificationName: string, send_toolBarEditorName: string, status: number) => {
    const copyToolBarClassification = Object.assign(toolBarClassificationArray);
    copyToolBarClassification[send_toolBarClassificationName].toolBarEditorDict[send_toolBarEditorName].editorStatus = status;
    toolBarClassificationArraySetState(copyToolBarClassification);
  };
  ...
  const componentConvertToolBarEditor = (send_toolBarClassificationName: string) => {
    const componentConvertToolBarTemp = [];

    if (toolBarClassificationArray[send_toolBarClassificationName] === undefined) {
      return [];
    }

    const toolBarEditorDictValue = Object.values(toolBarClassificationArray[send_toolBarClassificationName].toolBarEditorDict);
    ...
    <div>
      <SetupToolbarContext.Provider
        value={{
          ...
          componentConvertToolBarClassification: componentConvertToolBarClassification,
