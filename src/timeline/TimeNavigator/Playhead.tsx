...
export const TimeNavigatorPlayheadComponent = () => {
  ...
  const playheadViewPos = TimeNavigatorContextValue.timelimeRender.playheadViewPos;
  ...
  const MouseDown = (event: any) => {
    ...
    TimeNavigatorContextValue.timelimeRenderSetState({ type: "playheadMove", playheadViewPos: mousePushPos });
  };
  const MouseMove = (event: any) => {
    ...
    if (userHandPlayhead.mouseDownFlag === 1) {
      ...
      TimeNavigatorContextValue.timelimeRenderSetState({ type: "playheadMove", playheadViewPos: mouseX });
    }
  };
  const MouseRelease = () => {
  ...
  return (
    <div className="timeNavigator-playhead-area" ref={timeNavigatorPlayheadAreaElement}>
      <div className="timeNavigator-playhead" style={{ left: playheadViewPos - 21 }}>
        <p>{Math.floor(AppContextValue.getCompositePlayheadTimePos(SetupEditorContextValue.choiceComposite))}</p>
      </div>
    </div>
  );
};
